...
export class ProductsService {
  ...
  async create(createProductDto: CreateProductDto) {
    try {
      ...
      const product: Product = this.productRepository.create({
        ...productDetails, images: images.map(image => this.productImageRepository.create({
          url: image
        }))
      });
  ...
  async findOne(term: string) {
    try {
      ...
      if (isUUID(term)) {
        ...
        product = await queryBuilder.where('UPPER(title) = :title or slug = :slug', { slug: term.toLocaleLowerCase(), title: term.toUpperCase() }).leftJoinAndSelect('prod.images', 'prodImages').getOne();
      }
  ...
  async findOnePlain(term: string) {
    const { images = [], ...rest } = await this.findOne(term) as Product;
