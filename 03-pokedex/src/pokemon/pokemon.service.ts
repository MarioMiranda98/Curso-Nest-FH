...
export class PokemonService {
  ...
  async findOne(term: string): Promise<Pokemon> {
    ...
    if (!pokemon) {
      throw new NotFoundException(`Pokemon with id, name or no ${term} does not found`);
    }
  ...
  async update(term: string, updatePokemonDto: UpdatePokemonDto) {
    ...
    if (!pokemon) throw new BadRequestException(`Pokemon with term ${term} does not exist`);
